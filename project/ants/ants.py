...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            is_self_container = isinstance(self, ContainerAnt)
            is_place_container = isinstance(place.ant, ContainerAnt)
            if is_self_container and is_place_container:
                assert False, 'Two ants in {0}'.format(place)
            elif is_place_container:
                if place.ant.can_contain(self):
                    place.ant.contain_ant(self)
                else:
                    assert False, 'Two ants in {0}'.format(place)
            elif is_self_container:
    ...
    def nearest_bee(self, beehive):
        ...
        for _ in range(self.min_range):
            ...
            if here == beehive:
                return None
            distance += 1
        while not find_bee(here): 
            here = here.entrance 
            distance += 1
            if here == beehive or distance > self.max_range:
                return None
        return random_or_none(here.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        here = self.place
        for bee in here.bees.copy():
            bee.armor -= amount

        if self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
            for bee in here.bees:
                bee.armor -= self.damage

        for bee in here.bees.copy():
            if bee.armor <= 0:
                here.remove_insect(bee)
                bee.death_callback()
...
class ContainerAnt(Ant):
    ...
    def contain_ant(self, ant):
        ...
        if self.can_contain(ant):
           self.contained_ant = ant
            ...
            assert False, "{} has contained {}".format(self, self.contained_ant)
        # END Problem 9
    ...
    def action(self, gamestate):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if not self.contained_ant == None:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if not QueenAnt.first_queen_appear:
            QueenAnt.first_queen_appear = True
            self.queen = True
        else:
            self.queen = False
    ...
    def action(self, gamestate):
        ...
        if self.queen:
            ...
            while here.exit:
                here = here.exit
                if isinstance(here.ant, ContainerAnt):
                    give_buff(here.ant.contained_ant)                   
                give_buff(here.ant)
...
def apply_effect(effect, bee, duration):
    ...
    def apply_new_action(gamestate):
        nonlocal duration
        if duration > 0:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        if isinstance(here.ant, ContainerAnt):
            insects_distance[here.ant] = distance
        if here.bees:
            for bee in here.bees.copy():
                insects_distance[bee] = distance
        here = here.entrance
        distance += 1
        ...
        return insects_distance
        # END Problem OPTIONAL

    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage = self.base_damage-0.2*distance-0.05*self.insects_shot
        if damage < 0:
            damage = 0
        return damage
        # END Problem OPTIONAL
