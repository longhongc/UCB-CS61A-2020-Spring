...
def closer_city(lat, lon, city1, city2):
    ...
    return (get_name(city2), get_name(city1))[distance(temp_city, city1) < distance(temp_city, city2)]
...
def nut_finder(t):
    ...
    for branch in branches(t):
        if tree('nut') in branch:
            return True
        return nut_finder(branch) 
    return False
...
def sprout_leaves(t, values):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(label(t), [tree(n) for n in values])
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        return tree(label_sum)
    if is_leaf(t1):
        return tree(label_sum, branches(t2))
    if is_leaf(t2):
        return tree(label_sum, branches(t1))

    long_b = branches((t2,t1)[len(branches(t1)) > len(branches(t2))])
    small_b = branches((t1,t2)[len(branches(t1)) > len(branches(t2))])

    return tree(label_sum, [add_trees(branches(t1)[i], branches(t2)[i]) for i in range(len(small_b))] + long_b[len(small_b):])
