...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        add_attack = other_card.attack
        add_defense = other_card.defense
        for card in player.deck.cards:
            card.attack += add_attack
            card.defense += add_defense
        for card in opponent.deck.cards.copy():
            if card.attack == add_attack or card.defense == add_defense:
