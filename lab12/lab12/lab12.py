...
def prune_min(t):
    ...
    if t.is_leaf():
        return 
    assert len(t.branches)==2, "tree branches must be 2" 
    saved_branch_label = min([branch.label for branch in t.branches])
    for branch in t.branches.copy():
        if branch.label == saved_branch_label:
            t.branches = [branch]
...
def remainders_generator(m):
    ...
    def  helper():
        def remainders(n):
            ...
            while True:
                if n==0: 
                    yield (next(index)+1) * m
                else:
                    yield next(index) * m + n  
            
        remainder = natural()
        count = 0
        while m > count:
            yield remainders(next(remainder))
            count+=1
    yield from helper()
...
def mapl(lst, fn):
    ...
    if lst.rest is Link.empty:
        return helper(lst.first, Link.empty) 
    else:
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return  lst if lst is Link.empty else foldl(lst, lambda l, n: Link(n, l), Link.empty)
    if lst is Link.empty:
        return lst
    def helper(r, l):
        if r is Link.empty:
            ...
            return helper(r.rest, Link(r.first, l))

    return helper(lst.rest, Link(lst.first))
...
def num_splits(s, d):
    ...
    def splits_list(lst):
        if len(lst)==1:
            return [lst, []]
        return [l+[lst[0]] for l in splits_list(lst[1:])] + [l for l in splits_list(lst[1:])]
    count = 0
    for l in splits_list(s):
        if check_within(s, l):
            count+=1
    return int(count/2)
